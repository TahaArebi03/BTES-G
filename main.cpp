...
class Seat {
private:
    int seatNumber;       // —ﬁ„ «·„ﬁ⁄œ
    bool isReserved;      // Õ«·… «·ÕÃ“ („ÕÃÊ“ √Ê „ «Õ)
    int eventID;          // —ﬁ„ «·ÕœÀ «·„— »ÿ »«·„ﬁ⁄œ
    string reservedBy;    // «”„ «·„” Œœ„ «·–Ì ÕÃ“ «·„ﬁ⁄œ
...
public:
    ...
    bool operator==(const Event& other) const {
        return eventID == other.eventID; // „ﬁ«—‰… »«” Œœ«„ ID
    }
...
public:
    ...
    NotificationManager notificationManager; // ≈÷«›… NotificationManager ··„” Œœ„
    ...
    static User createUser(int id) {
        ...
        User newUser(username, password, id, 1000.0); //  ⁄ÌÌ‰ —’Ìœ √Ê·Ì 1000
        users.push_back(newUser); // ≈÷«›… «·„” Œœ„ ≈·Ï «·ﬁ«∆„…
        return newUser;
    }
    ...
    for (const auto& booking : bookings) {
        ...
        if (event) {
            // ⁄—÷ «·„ﬁ«⁄œ «·„ÕÃÊ“… ›Ì «·ÕœÀ
            cout << "Reserved Seats: ";
            for (const auto& seat : event->getSeats()) { // «” Œœ«„ «·œ«·… «·ÃœÌœ…
...
public:
    ...
    void adminMenu(NotificationManager& nm) { //  „—Ì— NotificationManager
        int choice;
        while (true) {
            cout << "\n--- Admin Menu ---\n";
            cout << "1. Add Event\n";
    ...
    while (true) {
        ...
        switch (choice) {
            ...
            case 2: {
                ...
                string adminPassword = "1234"; // ﬂ·„… «·„—Ê— «·«› —«÷Ì…
                Admin admin(adminUsername, adminPassword);

                if (admin.login()) {
                    admin.adminMenu(nm); // œŒÊ· «·„œÌ—
                } else {
